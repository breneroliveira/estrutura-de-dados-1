...
/// Enqueue
bool enfileiraF(Fila *f, int cod, int pas, string emp) /// Incluir valor na fila
{
...
void mostraF(Fila *f)
{
    ...
    if (!vaziaF(f))
    {
        ...
        {
            cout << "{" << f->dados[idx].codigo << ", " << f->dados[idx].passageiros << ", " << f->dados[idx].empresa << "}";
...
void mostraVetorDadosF(Fila *f)
{
    ...
    for(int i = 0; i < f->tam; i++)
    {
        ...
        if(dadosFila[i])
            cout << f->dados[i].codigo << " | " << f->dados[i].passageiros << " | " << f->dados[i].empresa;
        else
            cout << "-";
